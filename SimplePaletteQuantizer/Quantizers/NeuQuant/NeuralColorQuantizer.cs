...
namespace SimplePaletteQuantizer.Quantizers.NeuQuant
{
    ...
    public class NeuralColorQuantizer : BaseColorQuantizer
    {
        ...
        const byte DefaultQuality = 10; // 10

        const int AlphaBiasShift = 10;
        const int AlphaRadiusBias = 1 << AlphaRadiusBetaShift;
        const int AlphaRadiusBetaShift = AlphaBiasShift + RadiusBiasShift;
        const int Beta = (InitialBias >> BetaShift);
        const int BetaShift = 10;
        const int BetaGamma = (InitialBias << (GammaShift - BetaShift));
        const int DefaultRadius = NetworkSize >> 3;
        const int DefaultRadiusBiasShift = 6;
        const int DefaultRadiusBias = 1 << DefaultRadiusBiasShift;
        const int GammaShift = 10;
        const int InitialAlpha = 1 << AlphaBiasShift;
        const int InitialBias = 1 << InitialBiasShift;
        const int InitialBiasShift = 16;
        const int InitialRadius = (DefaultRadius * DefaultRadiusBias);
        const int MaximalNetworkPosition = NetworkSize - 1;
        const int NetworkSize = 256;
        const int NetworkBiasShift = 4;
        const int RadiusBiasShift = 8;
        const int RadiusDecrease = 30;
        ...
        readonly ConcurrentDictionary<int, bool> uniqueColors;

        int[] bias;
        int[] frequency;
        int[] networkIndexLookup;
        int[] radiusPower;
        ...
        int FindClosestNeuron(int red, int green, int blue)
        {
        ...
        /// </summary>
        void LearnNeuron(int alpha, int red, int green, int blue, int networkIndex)
        {
            /* alter hit neuron */
        ...
        void LearnSampleColor(Color color)
        {
